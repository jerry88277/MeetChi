...
## 5. 桌面端功能設計 (Desktop Client Functional Design)

本章節定義基於 **Tauri** 框架重構後的桌面端功能模組，取代原有的 Electron 設計。

### 5.1 應用程式架構 (App Architecture)
- **核心框架**: **Tauri v2** (Rust Core + Web Frontend)
- **前端 (View)**: Next.js (SSG)
    - 負責渲染 UI、字幕顯示、設定表單。
    - 不處理任何音訊 I/O 或網路傳輸，僅透過 Tauri Events 接收狀態與文字。
- **後端 (Core)**: Rust
    - 負責系統級操作：音訊採集、VAD 運算、gRPC 傳輸、視窗管理。
    - 優勢：極低記憶體佔用、真正的多執行緒、直接調用 OS API。

### 5.2 音訊捕獲與處理模組 (Rust Audio Module)
- **音訊源**: 使用 `cpal` (Cross-platform Audio Library) 獲取系統音訊 (Loopback) 或麥克風輸入。
- **VAD 引擎**: 整合 **Silero VAD** (透過 `ort` ONNX Runtime)。
    - 在 Rust 層直接對 PCM 數據進行 VAD 判斷。
    - 僅當 `speech_prob > threshold` 時，才將數據放入發送隊列。
- **網路傳輸**: 使用 `tonic` (gRPC client)。
    - 建立與 Cloud Run 的雙向串流 (Bi-directional Streaming)。
    - 傳送: `AudioFrame` (PCM bytes)。
    - 接收: `TranscriptResponse` (Text, Translation)。

### 5.3 視窗管理模組 (Rust Window Manager)
- **多視窗策略**:
    - `main` (Overlay): 全螢幕、透明、無邊框、永遠置頂。
    - `settings` (Settings): 標準視窗、有標題列、不透明。
- **穿透邏輯 (Click-through)**:
    - 預設狀態 (`IgnoreMouseEvents`): 視窗對滑鼠事件透明。
    - 攔截狀態: 當前端發出 `mouseenter` 事件 (使用者游標進入控制列區域) 時，Rust 暫時關閉穿透屬性。
    - 恢復狀態: 當前端發出 `mouseleave` 事件時，Rust 重新啟用穿透屬性。

### 5.4 狀態同步 (State Sync)
- **Tauri Events**:
    - `backend-status`: Rust -> Frontend (連線中、錄音中、錯誤)。
    - `transcript-update`: Rust -> Frontend (收到新的字幕片段)。
    - `config-changed`: Frontend -> Rust (使用者修改了設定)。
    - `window-control`: Frontend -> Rust (最小化、關閉)。
