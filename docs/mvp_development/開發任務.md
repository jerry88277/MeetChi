...
### Sprint 4.8: Tauri 音訊擷取與 VAD 整合 (Tauri Audio Capture & VAD) - Completed
- [x] **後端開發 (Rust)**:
    - **音訊串流**: 使用 `cpal` 建立 Input Stream，擷取系統音訊 (Loopback) 或麥克風 PCM 數據。
    - **VAD 整合**: 整合 `webrtc-vad` 模型進行本地端語音偵測 (取代原定的 ONNX 方案以簡化依賴)。
    - **緩衝機制**: 實作 Ring Buffer 處理音訊切分。
- [x] **網路傳輸**:
    - 使用 `tokio-tungstenite` 建立 WebSocket 連線 (取代原定的 gRPC 方案以複用後端邏輯)。
    - 實作雙向串流，發送音訊並接收字幕。
- [x] **測試任務**:
    - 驗證系統音訊錄製的音質與 VAD 準確度。
    - 驗證長時間錄音的穩定性與記憶體佔用。

### Sprint 4.9: 系統整合與優化 - Completed
- [x] **UI/UX 優化**: 
    - 實作自定義標題列與絕對定位層級，解決拖曳問題。
    - 啟用 JS 驅動的視窗縮放，解決 Windows 無邊框限制。
    - 優化設定頁面體驗 (即時預覽、動態創建)。
- [x] **前後端聯調**: 
    - 確保 Rust 前端與 Python 後端 (FastAPI) 透過 WebSocket 穩定通訊。
    - 驗證 ASR 轉錄結果與 LLM 潤飾結果在前端的即時顯示。
