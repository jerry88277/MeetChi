...
## 6. 桌面客戶端規範 (Desktop Client Specifications)

本章節定義基於 **Tauri** 框架重構後的桌面端應用程式需求，取代原有的 Electron 架構。

### 6.1 主視窗 (Main Window - Overlay)
- **視覺設計**:
    - **完全透明**: 預設狀態下，視窗背景必須完全透明，僅顯示字幕文字。文字需具備高對比度陰影 (Text Shadow) 以確保在任何背景上均可閱讀。
    - **無邊框**: 移除所有作業系統原生的標題列與邊框。
    - **永遠置頂 (Always on Top)**: 確保字幕始終浮動於其他應用程式（如 PowerPoint, Teams）之上。
- **互動行為**:
    - **穿透模式 (Click-through)**:
        - 當滑鼠位於「純字幕區域」時，點擊事件應直接穿透至下層應用程式。
        - 使用者應能正常操作位於字幕下方的視窗。
    - **控制模式 (Control Mode)**:
        - 當滑鼠移入視窗範圍時，顯示「控制列 (Control Bar)」與「拖曳手柄 (Drag Handle)」。
        - 此時視窗應**捕捉滑鼠事件**，允許使用者拖曳視窗位置或點擊控制按鈕。
        - 滑鼠移出後，自動恢復穿透模式。

### 6.2 設定視窗 (Settings Window)
- **架構變更**: 捨棄原有的 Modal 設計，改為**獨立的多視窗架構**。
- **視窗行為**:
    - 點擊主視窗的「設定 (Gear)」按鈕時，開啟一個全新的、獨立的 OS 視窗。
    - 此視窗**不透明**、擁有標準標題列、可自由縮放與移動。
    - 設定變更（如語言切換、字體大小）應即時反映至主視窗 (Overlay)。
- **功能收納**:
    - 音訊來源選擇 (系統音訊/麥克風)。
    - 語言與翻譯模式設定。
    - 外觀設定 (字體、透明度、行數)。
...
- **音訊管道**:
    - **Client-side VAD**: 使用 Rust (`ort` crate + Silero VAD) 進行本地端語音活動偵測，僅在有人聲時傳輸數據。
    - **傳輸**: 使用 Rust (`tonic` crate) 建立 gRPC 雙向串流直接連線至 Cloud Run，繞過瀏覽器限制。
