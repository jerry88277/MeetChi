...
import omegaconf.listconfig # Import omegaconf.listconfig
import omegaconf.base # Import omegaconf.base
import typing # Import typing for Any
import collections # Import collections for defaultdict
import omegaconf.nodes # Import omegaconf.nodes for AnyNode
import omegaconf.base # Import omegaconf.base
import torch.torch_version # Import TorchVersion
import pyannote.audio.core.model # Import pyannote.audio.core.model
import pyannote.audio.core.task # Import pyannote.audio.core.task

# Register omegaconf.listconfig.ListConfig, omegaconf.base.ContainerMetadata, typing.Any, list, collections.defaultdict, dict, int, omegaconf.nodes.AnyNode, omegaconf.base.Metadata, torch.torch_version.TorchVersion, pyannote.audio.core.model.Introspection, pyannote.audio.core.task.Specifications, pyannote.audio.core.task.Problem, and pyannote.audio.core.task.Resolution as safe globals for torch.load
torch.serialization.add_safe_globals([omegaconf.listconfig.ListConfig, omegaconf.base.ContainerMetadata, typing.Any, list, collections.defaultdict, dict, int, omegaconf.nodes.AnyNode, omegaconf.base.Metadata, torch.torch_version.TorchVersion, pyannote.audio.core.model.Introspection, pyannote.audio.core.task.Specifications, pyannote.audio.core.task.Problem, pyannote.audio.core.task.Resolution])
...
try:
    ...
    # Handle the case where segment['words'] is empty by splitting segment['text'] and assigning linear timestamps to each word.
    ...
    for segment in align_result['segments']:
        ...
        # 如果 words 已存在，就把每個 word 的內容轉成繁體
        if 'words' in segment and segment['words']:
            for word in segment['words']:
                if word.get('start', 0) == 0:
                    word['start'] = round(segment['start'], 3)
                if word.get('end', 0) == 0 or word['end'] < word['start']:
                    word['end'] = round(word['start'] + 0.0, 3)
                if 'word' in word:
                    word['word'] = cc.convert(word['word'])
        else:
            # 如果 words 是空的，就斷詞並補上時間資訊（線性分配）
            words = ws_driver([segment['text']])[0]  # 使用 CKIP 斷詞
            total = len(words)
            ...
            for i, word in enumerate(words):
                word_info = {
                    'word': cc.convert(word),  # 確保每個詞也轉成繁體
                    'start': round(start + i * interval, 3),
                    'end': round(start + (i + 1) * interval, 3),
                    'score': 0.0,
        ...
        processor = SubtitlesProcessor(
            [segment],
            lang=align_result['language'],
            max_line_length=100, # Modified to reduce sentence splitting
            min_char_length_splitter=24,
            is_vtt=False
        )
        subtitles = processor.process_segments(advanced_splitting=True)
...
# Save as TXT file
try:
    logger.info(f"Save result as a TXT file with timestamps: {tr_txt_path}")
    if diarize_toggle == "1":
        align_result_text = [
            f"[{format_time(itr['start'])} --> {format_time(itr['end'])}] {itr.get('speaker', 'Unknown Speaker')}: {itr['text']}"
            for itr in align_result["segments"]
            if 'text' in itr
        ]
    else:
        align_result_text = [
            f"[{format_time(itr['start'])} --> {format_time(itr['end'])}] {itr['text']}"
            for itr in align_result["segments"]
            if 'text' in itr
        ]
